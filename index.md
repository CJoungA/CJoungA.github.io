...
## [ 요구사항 6주차 ]
   ...
   4. 고유의 반동과 능력치를 가진 총기1을 제작한다.
   5. 적은 3개의 지점에서 생성된다.
   6. 이동 및 총기,적에게 효과음을 넣는다
   7. 일반, 이동속도강화, 체력강화 타입의 적이 있다.
   8. 적을 죽이면 점수가 올라간다.
   9. 재장전관련 사항을 만든다.
   10. 탄약의 총합이 0이거나 남은시간이 0이되면 게임을 종료한다.
  <br>
...
## [개발작업(1주)]
  ...
  해당 요구사항 : 플레이어 캐릭터의 기본적인 동작

  1) 플레이어 캐릭터 생성
  2) 마우스로 캐릭터 시점이동(1인칭시점)
  3) WASD로 이동
  4) 플레이어 공격모션
   
  작업 내용 : 

  ~~1. 캐릭터 모델과 무기모델을 추가하여 플레이어 캐릭터를 생성한다.~~ 
  
  ~~2. 마우스로 캐릭터 시점을 이동시키며 WASD로 이동한다.~~   
  
  ~~3. 플레이어가 마우스 좌클릭을 눌렀을 경우 총기 발사모션을 설정한다.~~
  
    ~~4. 총기 발사모션에 총기 사운드와 총구 이펙트를 추가한다.~~ 
...
~~1. 플레이어 캐릭터의 사운드를 추가한다.~~

~~2. 플레이어가 마우스 좌클릭을 눌렀을 경우 총기 발사모션을 설정한다.~~

~~3. 총기 발사모션에 총기 사운드와 총구 이펙트를 추가한다.~~
