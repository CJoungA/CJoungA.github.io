...
## [개발작업(1주)]
  ...
  1) 플레이어 캐릭터 생성 (완)
  2) 마우스로 캐릭터 시점이동(1인칭시점) (완)
  3) WASD로 이동 (완)
  4) 플레이어 공격모션 (미완)
   
  작업 내용 : 

  ~~1. 캐릭터 모델과 무기모델을 추가하여 플레이어 캐릭터를 생성한다. (완)~~
  ~~2. 마우스로 캐릭터 시점을 이동시키며 WASD로 이동한다.  (완)~~  
  ~~3. 플레이어가 마우스 좌클릭을 눌렀을 경우 총기 발사모션을 설정한다. (미완)~~
    4. 총기 발사모션에 총기 사운드와 총구 이펙트를 추가한다. (미완) 
  ~~5. 쉬프트를 눌러 느리게 걷기 추가 (완)~~
